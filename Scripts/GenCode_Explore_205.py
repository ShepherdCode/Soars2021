...
# ###Count the number of RNA sequences with no ORFs
# 
# ---
  ...
  for seq in sequences:
    if len(seq) > min_seq_len and len(seq) < max_seq_len:
      ...
      if max_orf_len >= min_max_orf_len and max_orf_len <= max_max_orf_len:
        count += 1
  print("Number of sequences with length between the bounds", min_seq_len, "and", max_seq_len, "with greatest ORF length within bounds of", min_max_orf_len, "and", max_max_orf_len, "is", count)
...
print("mRNA:")
count_constraint_valid_sequences(pcdf, 200, 12800, 0, 0)
print("lncRNA:")
count_constraint_valid_sequences(ncdf, 200, 12800, 0, 0)
show_time()
