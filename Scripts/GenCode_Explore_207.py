...
# ###Prepare data for CDF plot of sequence lengths
# 
# ---
...
pc_seqs = pcdf['sequence'].tolist()
nc_seqs = pcdf['sequence'].tolist()
pc_sequence_lengths = []
nc_sequence_lengths = []
for seq in pc_seqs:
  pc_sequence_lengths.append(len(seq))
for seq in nc_seqs:
  nc_sequence_lengths.append(len(seq))
...
def cdf_plot_compare(data_a, data_b, bins, title, x_label, y_label, a_name, b_name):
  plt.figure()

  hist_a = plt.hist(data_a, bins, density=True, histtype='step', cumulative=True, label=a_name)
  hist_b = plt.hist(data_b, bins, density=True, histtype='step', cumulative=True, label=b_name)
...
#CDF plot
#Crashes Colab session because it uses too much ram
#cdf_plot_compare(pc_seqs, nc_seqs, 50, "Insert Title Here", "Sequences Lengths", "Probability", "mRNA", "lncRNA")
...
bar_plot_compare(pc_max_orf_len_less_than_100, nc_max_orf_len_less_than_100, "Number of Sequences of Max ORF Length Equal to or Less than 100", "Sequence Length Range", "Number of Sequences", bar_x_axis_labels, "mRNA", "lncRNA")

#Heatmap
plt.imshow(hm_data, cmap='hot', interpolation='nearest')
plt.xlabel('Sequence Length Ranges')
plt.xticks(np.arange(num_bins), hm_x_axis_labels, rotation=45, ha='right')
plt.yticks(np.arange(num_bins), ["mRNA Mean Longest ORF Length", "mRNA Mean Number of Non-contained ORFs", "mRNA Mean Number of Contained ORFs", "lncRNA Mean Longest ORF Length", "lncRNA Mean Number of Non-contained ORFs", "lncRNA Mean Number of Contained ORFs"])
plt.show()
